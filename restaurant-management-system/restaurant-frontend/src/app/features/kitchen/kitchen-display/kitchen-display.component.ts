...
@Component({
  ...
  imports: [NgFor, NgIf, TranslateModule, MatButtonModule, PageHeaderComponent, RmsIconBtnComponent, EnumTranslatePipe, LocalizedNamePipe],
...
export class KitchenDisplayComponent implements OnInit, OnDestroy {
  ...
  readonly columns = [
    ...
    { titleKey: 'KITCHEN.NEW_ORDERS', statuses: ['PENDING', 'ACCEPTED'], actionKey: 'KITCHEN.ADVANCE' },

    { titleKey: 'KITCHEN.IN_PROGRESS', statuses: ['PREPARING'], actionKey: 'KITCHEN.ADVANCE' },

    { titleKey: 'KITCHEN.READY_TO_SERVE', statuses: ['READY'], actionKey: 'KITCHEN.DONE' }
  ...
  load(): void {
    ...
    this.kitchen.getActiveOrders(this.branchCtx.activeBranchId).subscribe({
  ...
  get newCount(): number {
    ...
    return this.ordersByStatuses(this.newStatuses).length;
  ...
  get progressCount(): number {
    ...
    return this.ordersByStatuses(this.progressStatuses).length;
  ...
  get readyCount(): number {
    ...
    return this.ordersByStatuses(this.readyStatuses).length;
